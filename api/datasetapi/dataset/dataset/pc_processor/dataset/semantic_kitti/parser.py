...
class SemanticKitti(object):
    ...
    def loadDataByKey(self, seq_id, frame_id):
        pointcloud_path = os.path.join(self.root, seq_id, "velodyne", f"{frame_id}.bin")
        label_path = os.path.join(self.root, seq_id, "labels", f"{frame_id}.label")
        pointcloud = self.readPCD(pointcloud_path)
        if self.has_label:
